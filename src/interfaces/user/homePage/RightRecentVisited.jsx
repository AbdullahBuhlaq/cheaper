        ...
        <div className="received-item-line">
          <div className="progress-line">
            ...
            <span className="time end">
              {new Date(
                new Date(props.item.createdAt).setDate(
                  new Date(props.item.createdAt).getDate() + 2
                )
              ).toLocaleDateString()}{" "}
            </span>
          </div>
          <div className="received-items-content">
            <div className="received-files">
              ...
              {Array.from(Array(3 - props.item.story.length).keys()).map(
                (item, index) => {
                  return (
                    <motion.div
                      className="image-wrapper"
            ...
            <div className="received-files-info">
              لقد قمت بزيارة{" "}
              <span className="info-purple">{props.item.namStore}</span> في
              تاريخ{" "}
              <span className="info-purple">
                {new Date(props.item.dateTake).toLocaleDateString()}{" "}
              </span>
            </div>
          </div>
        </div>
