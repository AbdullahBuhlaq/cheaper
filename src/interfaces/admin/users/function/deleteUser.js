...
async function deleteUserFunc(id, userInformation, setUserInformation, refreshStatus, setRefreshStatus, users, setUsers, toast) {
  try {
    ...
    if (data.success) {
      // delete users[id];
      setUsers({ ...users, [id]: { ...users[id], disableAt: new Date().toISOString() } });
      ...
      if (data.error == "jwt expired") {
        ...
        await deleteUserFunc(id, { ...userInformation, ...status }, setUserInformation, refreshStatus, setRefreshStatus, toast);
      } else {
