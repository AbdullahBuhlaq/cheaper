...
export default async function getStoreSpam(userInformation, setUserInformation, refreshStatus, setRefreshStatus, setUsers, users, toast, usersPage, setUsersPage, id) {
  try {
    setUsersPage({ ...usersPage, loadingNow: true });
    let url = `${import.meta.env.VITE_URL}/admin/stores/evaluation/?type=spam&storeId=${id}`;
    ...
    //   data: [
    //     {
    //       reasonSpam: "هاد الزلمي كذاب ما عطاني العرض",
    ...
    if (data.success) {
      ...
      if (data.error == "jwt expired") {
        ...
        await getStoreSpam({ ...userInformation, ...status }, setUserInformation, refreshStatus, setRefreshStatus, setUsers, users, toast, usersPage, setUsersPage, id);
      } else {
