...
export default async function getAcceptedStores(
  userInformation,
  setUserInformation,
  refreshStatus,
  setRefreshStatus,
  ...
  try {
    ...
    if (data.success) {
      if (!data.data.data.length) {
        ...
        if (acceptedStores == -1 || acceptedStoresPage.page == 1)
          setAcceptedStores({});
      } else {
        let finalAcceptedStores = {};
        await Promise.all(
          data.data.data.map(async (acceptedStore) => {
            finalAcceptedStores[acceptedStore.id] = { ...acceptedStore };
          })
        );

        if (acceptedStoresPage.page == 1)
          setAcceptedStores({ ...finalAcceptedStores });
        else setAcceptedStores({ ...acceptedStores, ...finalAcceptedStores });

        setAcceptedStoresPage({
          ...acceptedStoresPage,
          search: false,
          page: acceptedStoresPage.page + 1,
          loadMore: true,
          loadingNow: false,
          OnlyClick: false,
        });
