...
export default async function getPendingStores(
  userInformation,
  setUserInformation,
  refreshStatus,
  setRefreshStatus,
  ...
  try {
    ...
    if (data.success) {
      if (!data.data.data.length) {
        ...
        if (pendingStores == -1 || pendingStoresPage.page == 1)
          setPendingStores({});
      } else {
        let finalPendingStores = {};
        await Promise.all(
          data.data.data.map(async (pendingStore) => {
            finalPendingStores[pendingStore.id] = { ...pendingStore };
          })
        );

        if (pendingStoresPage.page == 1)
          setPendingStores({ ...finalPendingStores });
        else setPendingStores({ ...pendingStores, ...finalPendingStores });

        setPendingStoresPage({
          ...pendingStoresPage,
          search: false,
          page: pendingStoresPage.page + 1,
          loadMore: true,
          loadingNow: false,
          OnlyClick: false,
        });
